...
public class GUI extends JFrame {
	...
	public GUI() {
		...
		JTextArea introTwo = new JTextArea("Choose a command from the \"Commands\" menu to buy or sell an investment, "
				+ "update prices for all investments, get gain for the portfolio, search for relevant investments, or quit the program.");
		...
		Intro.add(introTwo);// add non-editable textarea to first page
		...
		/**
		 * Action Listener for update Page
		 */
		updateItem.addActionListener((ActionEvent e) -> {
			...
			update.getSymbolField().setText(data.get(0).getSymbol());
			update.getNameField().setText(data.get(0).getName());
			update.getNext().setEnabled(true);
			update.getPrev().setEnabled(true);
		...
		/**
		 * Action Listener for search Page
		 */
